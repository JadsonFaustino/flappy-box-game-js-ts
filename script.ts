...
const pipes: pipeElement[] = []; // Array para armazenar os canos
...
function generatePipes() {
    ...
    const pipeTop = createPipeElement('pipe', 0, window.innerWidth, pipeHeight) as pipeElement;
    const pipeBottom = createPipeElement('pipe', pipeHeight + 300, window.innerWidth, window.innerHeight - pipeHeight - 300) as pipeElement;
...
function createPipeElement(className:string, top: number, left: number, height: number) {
...
function isColliding(element1: HTMLElement, element2: HTMLElement) {
